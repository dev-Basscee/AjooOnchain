  ...
  const { data: groups, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("groups")
        .select("*")
        .order("created_at", { ascending: true }) // Maybe ascending for a different view
        .limit(2);
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-8">
      {groups.map((group) => {
        ...
        return (
          <Card key={group.id} className="w-full max-w-[470px] bg-[#1212124c] rounded-3xl overflow-hidden border-[0.2px] border-solid border-[#cbcfd299]">
            <CardContent className="p-8 space-y-6">
                    ...
                    {participantSlots.map((slot, index) => (
                      <div
                        ...
                        className={`w-3 h-3 rounded-md ${
                          slot.filled ? "bg-primary-300" : "bg-[#d9d9d9]"
                        }`}
                      />
                    ))}
                  </div>
              ...
              >
                {joiningGroupId === group.id ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : null}
                {joiningGroupId === group.id ? "JOINING..." : `JOIN CIRCLE - ${participantsTotal - participantsCurrent} SPOTS LEFT`}
              </Button>
            </CardContent>
          </Card>
        );
